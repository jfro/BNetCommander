...
- (NSData *)dataForType:(ResType)aType named:(NSString *)aName
{
	NSData	* theData = nil;
	short int anId = 0;
	if( operateOnResourceUsingFunction( fileReference, aType, aName, &anId, getDataFunction, (void*)&theData )  )
		return theData;
	else
		return nil;
}
...
- (BOOL)getId:(short int *)anId ofResourceType:(ResType)aType named:(NSString *)aName
{
	BOOL res;
	NSLog(@"Fetching ID");
	res = operateOnResourceUsingFunction( fileReference, aType, aName, anId, getIdFunction, NULL );
	NSLog(@"Got: %i",anId);
	return res;
}
